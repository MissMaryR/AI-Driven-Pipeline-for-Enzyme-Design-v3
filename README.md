...
The monomer sequence is **not** hardcoded by length. LigandMPNN emits the designed sequence on line 4 of each `.fa` file with one chain per colon-separated field (e.g. `AAAA:AAAA:AAAA` for a homotrimer). Because cross-chain symmetry was enforced at the MPNN step, all three chains are identical, so the script takes just the first field (everything before the first `:`) as the monomer sequence and then assembles the AF3 JSON with `NUM_CHAINS` copies of that monomer.
