...
echo "$sorted_results" | while IFS= read -r result; do
    ...
    # Line 4 is the full LigandMPNN designed sequence. For a symmetric
    # homotrimer the file contains one sequence per chain joined by ':',
    # e.g. "AAAA:AAAA:AAAA" — and because cross-chain symmetry was
    # enforced at the MPNN step, all three chains are identical. Take
    # only the first chain (everything before the first ':') and use
    # that as the monomer sequence.
    full_seq=$(sed -n '4p' "$fa_file_path")
    monomer_seq="${full_seq%%:*}"
    actual_len=${#monomer_seq}

    # Sanity check: the sequence must not still contain a ':' (would
    # indicate the split failed and we'd embed a colon into the JSON).
    if [[ "$monomer_seq" == *:* ]]; then
        echo "ERROR: top_${rank} monomer_seq still contains ':'. Aborting."
        exit 1
    fi
    if [[ $actual_len -eq 0 ]]; then
        echo "ERROR: top_${rank} got empty monomer sequence from $fa_file_path"
        exit 1
    fi
